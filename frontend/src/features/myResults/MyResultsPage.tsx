...
const MyResultsPage: React.FC = () => {
    ...
    useEffect(() => {
        resultService.getMyResults()
            ...
            .catch(error => {
                console.error("Greška pri preuzimanju rezultata:", error);
            })
            .finally(() => {
                setLoading(false);
            });
    ...
    const handleOpenDetailsModal = async (resultId: number) => {
        ...
        try {
            ...
            console.error("Greška pri preuzimanju detalja:", error);
        } finally {
    ...
    if (loading) {
        return <div className="my-results-container"><h2>Učitavanje rezultata...</h2></div>;
    }
